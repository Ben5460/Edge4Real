...
public class TCPControllerClient : Singleton<TCPControllerClient>
{
    ...
    void Start()
    {
        new Thread(() => {
            ...
            ConnectClient(serverIPAddress, port, clientId, model);
        }).Start();
    }
    ...
    public void AddModel(Vector3 postion)
    {
        model.transform.Add(postion);


    }
    public void AddModelRotation(Quaternion quaternion)
    {
        model.rotation.Add(quaternion);


    }

    private void ConnectClient(string server, int port, int clientId, Model2 model)
    {
        try
        {
            ...
            if(client.Connected)
            {
                ...
                {   
                    lock(safeLock)
                    {
                        if (working == false)
                        {
                            string payLoad = JsonUtility.ToJson(model);
                            ...
                            model.rotation.Clear();
                            model.transform.Clear();
                        }
